...
def generate_output_files(train_stats, best_model, out_path):
    """
    Description: This method generates the plots and confusion matrix from the training output

    :param best_model: best model from training
    :param train_stats: Training stats generated by the training loop
    :param out_path: Path to save the output files
    """
    #torch.save(best_model, f'{out_path}/model.pth')
    generate_train_test_plots(train_stats, out_path)
    build_confusion_matrix(train_stats, out_path)


def generate_train_test_plots(train_stats, out_path):
    # matplotlib.use('GTK')
    plt.clf()
...
def build_confusion_matrix(train_stats, out_path):
    """
    Description: This method builds the confusion matrix
    ...
    sn.heatmap(cmn, annot=True, fmt='.2f', xticklabels=[1, 3], yticklabels=[1, 3])
    plt.ylabel('Actual')
